...
class Args:
    ...
    vi_tau: float = 0.1
    """ Tau for VIOptimizer anchor regularization"""
    use_wnb: bool = False
...
# --- NO NORMALIZATION ---
# VIOptimizer WITHOUT gradient normalization:
# Applies raw gradient (no norm division) + anchor proximal regularization.
        ...
        with torch.no_grad():
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    # Use raw gradient WITHOUT normalization, then add anchor regularization
                    grad_regularized = p.grad + tau * (p.data - x_anchor)
...
if __name__ == "__main__":
    ...
    # Use VIOptimizerNoNorm: anchor regularization WITHOUT gradient normalization
    actor_optimizers = [
        VIOptimizerNoNorm(actor.parameters(), lr=args.learning_rate_actor, tau=args.vi_tau)
        for actor in actors
    ]
    ...
    if args.use_wnb:
        ...
        wandb.init(
            ...
            config=vars(args),
            name=f"IPPO-VI-NoNorm-{run_name}",
        )
    writer = SummaryWriter(f"runs/IPPO-VI-NoNorm-{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s"
