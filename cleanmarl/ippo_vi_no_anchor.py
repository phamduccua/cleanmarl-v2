...
class Args:
    ...
    vi_tau: float = 0.1
    """ Tau for VIOptimizer regularization (kept for CLI compatibility but anchor is disabled)"""
    use_wnb: bool = False
...
# --- NO ANCHOR ---
# VIOptimizer WITHOUT anchor regularization:
# Performs only normalized-gradient descent without the anchor proximal term.
class VIOptimizerNoAnchor(optim.Optimizer):
    ...
    def set_anchor(self):
        # No-op: anchor is disabled in this variant
        pass
        ...
        with torch.no_grad():
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    # Normalized gradient descent only — no anchor proximal term
                    grad_norm = torch.norm(p.grad) + 1e-8
                    normalized_grad = p.grad / grad_norm
                    p.data.add_(normalized_grad, alpha=-lr)
...
if __name__ == "__main__":
    ...
    # Use VIOptimizerNoAnchor: normalized gradient descent WITHOUT anchor regularization
    actor_optimizers = [
        VIOptimizerNoAnchor(actor.parameters(), lr=args.learning_rate_actor, tau=args.vi_tau)
        for actor in actors
    ]
    ...
    if args.use_wnb:
        ...
        wandb.init(
            ...
            config=vars(args),
            name=f"IPPO-VI-NoAnchor-{run_name}",
        )
    writer = SummaryWriter(f"runs/IPPO-VI-NoAnchor-{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s"
    ...
    while step < args.total_timesteps:
        ...
        #####  Compute the advantage using A(s,a) = λ-Returns -V(s), see page 47 in David Silver's lecture n 4
        ...
        for actor_optimizer in actor_optimizers:
            actor_optimizer.set_anchor()  # No-op in this variant
