...
# ---------------------------------------------------------------------------
# Extragradient Optimizer (Korpelevich, 1976)
#
# Mỗi lần gọi optimizer.step() thực hiện 2 bước:
#   1. Extrapolation (look-ahead):
#        θ_half = θ - lr * ∇L(θ)      ← sao lưu θ, cập nhật sang θ_half
#   2. Update thực sự:
#        Tính ∇L(θ_half) bằng cách gọi closure()
#        θ = θ_saved - lr * ∇L(θ_half)
#
# Vì vậy mỗi step() cần closure để tính lại loss tại θ_half.
# ---------------------------------------------------------------------------
class ExtraAdamOptimizer(optim.Optimizer):
    """
    Extragradient optimizer sử dụng Adam làm inner optimizer.

    API:
        optimizer.extrapolation_step(closure)
            - Lưu θ_saved = θ hiện tại
            - Tính grad tại θ (qua closure)
            - Cập nhật Adam state và nhảy sang θ_half

        optimizer.update_step(closure)
            - Tính grad tại θ_half (qua closure)
            - Phục hồi θ từ θ_saved
            - Áp dụng Adam update dùng grad(θ_half)
    """
    ...
    # ------------------------------------------------------------------
    # Bước 1: Extrapolation  θ → θ_half
    # ------------------------------------------------------------------
    def extrapolation_step(self, closure):
        """Tính grad tại θ, lưu θ_saved, nhảy sang θ_half dùng Adam."""
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Lưu điểm hiện tại θ trước khi nhảy
                state["param_saved"] = p.data.clone()

                # θ_half = θ - lr_adam * m̂ / (√v̂ + ε)
                p.data.addcdiv_(exp_avg, denom, value=-step_size)

        return loss

    # ------------------------------------------------------------------
    # Bước 2: Update thực sự  θ ← θ_saved - lr * grad(θ_half)
    # ------------------------------------------------------------------
    def update_step(self, closure):
        """Tính grad tại θ_half, phục hồi θ_saved, áp dụng Adam update."""
        with torch.enable_grad():
            loss = closure()
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Cập nhật moment với grad(θ_half) — KHÔNG tăng step
                exp_avg_half = beta1 * exp_avg + (1 - beta1) * grad
                exp_avg_sq_half = beta2 * exp_avg_sq + (1 - beta2) * grad * grad

                bias_c1 = 1 - beta1 ** state["step"]
                bias_c2 = 1 - beta2 ** state["step"]
                step_size = lr / bias_c1
                denom = (exp_avg_sq_half.sqrt() / (bias_c2 ** 0.5)).add_(eps)

                # Phục hồi θ từ điểm đã lưu
                p.data.copy_(state["param_saved"])

                # θ_new = θ_saved - lr_adam * m̂_half / (√v̂_half + ε)
                p.data.addcdiv_(exp_avg_half, denom, value=-step_size)

                # Cập nhật state Adam để các bước tiếp theo nhất quán
                state["exp_avg"].copy_(exp_avg_half)
                state["exp_avg_sq"].copy_(exp_avg_sq_half)

        return loss

    def step(self, closure=None):
        """Không dùng — dùng extrapolation_step / update_step thay thế."""
        raise NotImplementedError(
            "ExtraAdamOptimizer: use extrapolation_step() + update_step() instead of step()."
        )
...
class Args:
    ...
    optimizer: str = "Adam"
    """ The optimizer (dùng cho critic và best-response)"""
    learning_rate_actor: float = 0.0008
    """ Learning rate for the actor (ExtraAdam)"""
    learning_rate_critic: float = 0.0008
    """ Learning rate for the critic"""
    extra_betas: tuple = (0.9, 0.999)
    """ Adam betas cho ExtraAdam actor optimizer"""
...
# ---------------------------------------------------------------------------
# Helper: tính actor loss cho 1 agent tại batch hiện tại
# Dùng để tạo closure cho ExtraAdam
# ---------------------------------------------------------------------------
def compute_actor_loss(actor, b_obs, b_avail_actions, b_actions, b_log_probs,
                       advantages, b_mask, args, device, agent_idx):
    agent_actor_loss = 0
    agent_entropy = 0.0
    agent_kl_divergence = 0.0
    agent_clipped_ratio = 0.0
    ...
    total_steps = b_mask.sum()
    agent_actor_loss = agent_actor_loss / total_steps
    agent_entropy = agent_entropy / total_steps
    agent_kl_divergence = agent_kl_divergence / total_steps
    agent_clipped_ratio = agent_clipped_ratio / total_steps
    return agent_actor_loss, agent_entropy, agent_kl_divergence, agent_clipped_ratio
...
if __name__ == "__main__":
    ...
    # Actor optimizers: ExtraAdam (extragradient)
    actor_optimizers = [
        ExtraAdamOptimizer(
            actor.parameters(),
            lr=args.learning_rate_actor,
            betas=args.extra_betas,
        )
        for actor in actors
    ]

    # Critic optimizer: standard Adam (hoặc optimizer được chỉ định)
    CriticOptimizer = getattr(optim, args.optimizer)
    critic_optimizer = CriticOptimizer(critic.parameters(), lr=args.learning_rate_critic)
    ...
    if args.use_wnb:
        ...
        wandb.init(
            ...
            config=vars(args),
            name=f"IPPO-ExtraGrad-{run_name}",
        )
    writer = SummaryWriter(f"runs/IPPO-ExtraGrad-{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s"
    ...
    while step < args.total_timesteps:
        # ----------------------------------------------------------------
        # Rollout collection (giống hệt ippo.py)
        # ----------------------------------------------------------------
        num_episode = 0
        ...
        # ----------------------------------------------------------------
        # Compute TD(λ) returns và advantages
        # ----------------------------------------------------------------
        ...
        # ----------------------------------------------------------------
        # Training loop — Extragradient cho actor, standard step cho critic
        # ----------------------------------------------------------------
        actor_losses = []
        critic_losses = []
        entropies_bonuses = []
        ...
        for _ in range(args.epochs):
            ...
            # ---- Actor update: Extragradient (2 closure calls per agent) ----
            for agent_idx, actor in enumerate(actors):
                opt = actor_optimizers[agent_idx]

                # --- Bước 1: Extrapolation step ---
                def make_closure(act, aidx):
                    def closure():
                        act.zero_grad()
                        loss, _, _, _ = compute_actor_loss(
                            act, b_obs, b_avail_actions, b_actions, b_log_probs,
                            advantages, b_mask, args, device, aidx
                        )
                        loss.backward()
                        if args.clip_gradients > 0:
                            torch.nn.utils.clip_grad_norm_(
                                act.parameters(), max_norm=args.clip_gradients
                            )
                        return loss
                    return closure

                closure_fn = make_closure(actor, agent_idx)
                opt.extrapolation_step(closure_fn)

                # --- Bước 2: Update step tại θ_half ---
                closure_fn2 = make_closure(actor, agent_idx)
                loss_at_half = opt.update_step(closure_fn2)

                # Logging (tính lại loss ở θ_new để ghi)
                with torch.no_grad():
                    final_loss, final_ent, final_kl, final_cr = compute_actor_loss(
                        actor, b_obs, b_avail_actions, b_actions, b_log_probs,
                        advantages, b_mask, args, device, agent_idx
                    )
                actor_gradient = norm_d([p.grad for p in actor.parameters() if p.grad is not None], 2)
                current_actor_losses.append(final_loss.item())
                current_actor_gradients.append(actor_gradient.item())
                entropies += final_ent.item()
                kl_divergence += final_kl.item()
                clipped_ratio += final_cr.item()

            # ---- Critic update: standard step ----
            critic_loss = 0
